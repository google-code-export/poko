...
class site_cms_common_Definition {
	public function __construct($id) {
		...
		$this->primaryKey = "";
		$this->params = new site_cms_common_DefinitionParams();
		$this->load();
		;
	}}
	public $id;
	...
	public function reOrderElements($order) {
		...
		$this->elements->sort(array(new _hx_lambda(array(&$order), "site_cms_common_Definition_0"), 'execute'));
		$this->save();
		$this->load();
		;
	}
	public function load() {
		...
		$this->allowCsv = $results->allowCsv;
		...
		try {
			$this->params = haxe_Unserializer::run($results->params);
			;
		}catch(Exception $»e) {
		...
		unset($results,$e2,$e,$autoOrdering);
	}
	public function save() {
		...
		$data->params = haxe_Serializer::run($this->params);
...
;
function site_cms_common_Definition_0(&$order, $el1, $el2) {
{
	return site_cms_common_Definition_1($el1, $el2, $order);
	;
}
}
function site_cms_common_Definition_1(&$el1, &$el2, &$order) {
