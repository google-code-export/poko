...
class site_examples_ComplexData extends site_examples_templates_DefaultTemplate {
	...
	public function main() {
		...
		$this->form1->addElement(new poko_form_elements_Selectbox("project", "Project", null, null, null, null), null);
		...
		$options = $this->app->getDb()->request("SELECT `id` as 'value', `name` as 'key' FROM example_projects");
		$»it = $options->iterator();
		while($»it->hasNext()) {
		$option = $»it->next();
		$this->form1->getElementTyped("project", _hx_qtype("poko.form.elements.Selectbox"))->addOption($option);
		}
