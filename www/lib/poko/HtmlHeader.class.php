...
class poko_HtmlHeader {
	...
	public function getJs() {
		...
		$jsBindings = poko_Application::$instance->request->jsBindings;
		$»it2 = $jsBindings->keys();
		while($»it2->hasNext()) {
		$jsBinding = $»it2->next();
		$str .= "<script> poko.js.JsApplication.instance.addRequest(\"" . $jsBinding . "\") </script> \x0A";
		}
		return $str;
	}
	public function getJsCalls() {
		$str = "";
		$jsCalls = poko_Application::$instance->request->jsCalls;
		$»it = $jsCalls->iterator();
		while($»it->hasNext()) {
		$jsCall = $»it->next();
	...
	function __toString() { return 'poko.HtmlHeader'; }
}
