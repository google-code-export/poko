...
class poko_views_renderers_HTemplate implements poko_views_Renderable{
	...
	public function render() {
		...
		if(file_exists($tpl)) {
			$renderer = new htemplate_Template(php_io_File::getContent($tpl));
			return $renderer->execute($this->data);
			unset($renderer);
		}
		else {
	...
	static function parse($template, $data) {
		$renderer = new htemplate_Template(php_io_File::getContent($template));
