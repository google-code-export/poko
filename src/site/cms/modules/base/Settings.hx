...
class Settings extends SettingsBase
{
	...
	{	
		...
		section = application.params.get('section');
	...
	{			
		...
		{
			case "main":
				var data = application.db.requestSingle("SELECT `value` FROM _settings WHERE `key`='cmsTitle'");
				form.addElement(new Input("cmsTitle", "CMS Title", data.value, true));
				data = application.db.requestSingle("SELECT `value` FROM _settings WHERE `key`='cmsLogo'");
			...
			case "ftp":
				...
				var data = application.db.requestSingle("SELECT * FROM _settings WHERE `key`='ftpUrl'");
				form.addElement(new Input("ftpUrl", "Url", data.value));
				var data = application.db.requestSingle("SELECT * FROM _settings WHERE `key`='ftpUsername'");
				form.addElement(new Input("ftpUsername", "Username", data.value));
				var data = application.db.requestSingle("SELECT * FROM _settings WHERE `key`='ftpPassword'");
				form.addElement(new Input("ftpPassword", "Password", data.value));
				var data = application.db.requestSingle("SELECT * FROM _settings WHERE `key`='ftpDirectory'");
	...
	private function saveDataFromForm():Void
	{
		...
		application.messages.addMessage("Settings saved.");
	}
}
