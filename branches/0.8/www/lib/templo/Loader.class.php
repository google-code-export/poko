...
class templo_Loader {
	...
	public function compileTemplate($path) {
		...
		if(file_exists($tmpFile)) {
			$macroStamp = templo_Loader_0($this, $path, $tmpFile);
			...
			unset($stamp,$sourceStamp,$macroStamp);
		}
		$result = 0;
		...
		unset($tmpFile,$result,$p,$code,$args);
	}
...
;
function templo_Loader_0(&$»this, &$path, &$tmpFile) {
if(templo_Loader::$MACROS !== null && file_exists(templo_Loader::$BASE_DIR . templo_Loader::$MACROS)) {
	return php_FileSystem::stat(templo_Loader::$BASE_DIR . templo_Loader::$MACROS)->mtime->getTime();
	;
}
}