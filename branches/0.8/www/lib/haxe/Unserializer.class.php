...
class haxe_Unserializer {
	...
	public function unserialize() {
		...
		case 121:{
			$len = $this->readDigits();
			if(_hx_char_code_at($this->buf, $this->pos++) !== 58 || $this->length - $this->pos < $len) {
		...
		case 115:{
			...
			return $bytes;
			unset($size,$rest,$max,$len,$i,$codes,$bytes,$buf,$bpos);
		}break;
		case 67:{
			$name = $this->unserialize();
			$cl = $this->resolver->resolveClass($name);
