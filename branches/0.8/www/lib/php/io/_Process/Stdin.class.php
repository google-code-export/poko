...
class php_io__Process_Stdin extends haxe_io_Output {
	...
	public function writeBytes($b, $pos, $l) {
		...
		if(feof($this->p)) {
			php_io__Process_Stdin_0($this, $b, $l, $pos, $s);
			;
		}
		$r = fwrite($this->p, $s, $l);
		if($r === false) {
			php_io__Process_Stdin_1($this, $b, $l, $pos, $r, $s);
			;
		}
		return $r;
		unset($s,$r);
...
;
function php_io__Process_Stdin_0(&$»this, &$b, &$l, &$pos, &$s) {
throw new HException(new haxe_io_Eof());
}
function php_io__Process_Stdin_1(&$»this, &$b, &$l, &$pos, &$r, &$s) {
