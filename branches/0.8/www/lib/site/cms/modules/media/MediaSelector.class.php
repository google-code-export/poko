...
class site_cms_modules_media_MediaSelector extends site_cms_templates_CmsPopup {
	...
	public function main() {
		...
		$dir = php_FileSystem::readDirectory($imageRoot);
		{
			...
			while($_g < $dir->length) {
				...
				if(is_dir(($imageRoot . "/") . $d) && !Lambda::has($exclude, $d, null)) {
					if($this->showOnlyLibraries->length === 0 || Lambda::has($this->showOnlyLibraries, $d, null)) {
		...
		$selector = new poko_form_elements_Selectbox("galleryList", "Select Gallery", $galleries, $this->gallery, null, null, null);
		...
		if($this->gallery !== null && $this->gallery !== "") {
			...
			{
				$_g = 0;
				while($_g < $dir1->length) {
					$d = $dir1[$_g];
					++$_g;
					if(!Lambda::has($exclude, $d, null)) {
						$this->items->add($d);
						;
					}
					unset($d);
		...
		unset($selector,$imageRoot,$galleries,$exclude,$dir);
	}
	public function getItem($name) {
		$onClick = site_cms_modules_media_MediaSelector_0($this, $name);
...
;
function site_cms_modules_media_MediaSelector_0(&$»this, &$name) {
switch($»this->from) {
case site_cms_modules_media_MediaSelector::$FROM_CMS:{
	return "window.top." . $»this->jsBindTop->getCall("updateItemFromMedia", new _hx_array(array($»this->elementId, $»this->gallery, $name)));
	;
}break;
case site_cms_modules_media_MediaSelector::$FROM_TINYMCE:{
	return ((("updateTinyMceWithValue('/res/media/galleries/" . $»this->gallery) . "/") . $name) . "');";
	;
}break;
case site_cms_modules_media_MediaSelector::$FROM_WYM:{
	return ((((("opener.document.getElementById('filebrowser').value = '" . $»this->baseUrl) . "res/media/galleries/") . $»this->gallery) . "/") . $name) . "'; window.close();";
	;
}break;
default:{
	return null;
