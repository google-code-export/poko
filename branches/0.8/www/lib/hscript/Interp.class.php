...
class hscript_Interp {
	...
	public function increment($e, $prefix, $delta) {
		...
		default:{
			throw new HException(hscript_Error::EInvalidOp(hscript_Interp_34($this, $delta, $e, $prefix)));
			;
		}break;
		}
		;
	...
	public function expr($e) {
		...
		$e1 = $»t->params[1]; $n = $»t->params[0];
		{
			...
			$this->locals->set($n, _hx_anonymous(array("r" => hscript_Interp_35($this, $e, $e1, $n))));
			return null;
			;
		}break;
		case 3:
		...
		{
			return hscript_Interp_36($this, $e, $e1, $e2, $econd);
			;
		}break;
		case 10:
		...
		$e1 = $»t->params[0];
		{
			throw new HException(hscript__Interp_Stop::SReturn(hscript_Interp_37($this, $e, $e1)));
			;
		}break;
		case 14:
		$name = $»t->params[2]; $fexpr = $»t->params[1]; $params = $»t->params[0];
		{
			...
			$me = $this;
			$f = array(new _hx_lambda(array(&$capturedLocals, &$e, &$fexpr, &$me, &$name, &$params), "hscript_Interp_38"), 'execute');
		...
		case 20:
		$ecatch = $»t->params[2]; $n = $»t->params[1]; $e1 = $»t->params[0];
		{
			$old = $this->declared->length;
			try {
				$v = $this->expr($e1);
...
function hscript_Interp_34(&$»this, &$delta, &$e, &$prefix) {
if($delta > 0) {
	return "++";
	;
}
...
function hscript_Interp_35(&$»this, &$e, &$e1, &$n) {
if($e1 === null) {
	return null;
	;
}
...
function hscript_Interp_36(&$»this, &$e, &$e1, &$e2, &$econd) {
...
function hscript_Interp_37(&$»this, &$e, &$e1) {
if($e1 === null) {
	return null;
	;
}
...
function hscript_Interp_38(&$capturedLocals, &$e, &$fexpr, &$me, &$name, &$params, $args) {
{
	...
	$me->locals = $me->duplicate($capturedLocals);
	{
		...
		while($_g1 < $_g) {
			$i = $_g1++;
			$me->locals->set($params[$i], _hx_anonymous(array("r" => $args[$i])));
			unset($i);
		}
		unset($_g1,$_g);
	}
